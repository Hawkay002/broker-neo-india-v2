...
function FeaturedGallery({ images, name }: { images: string[]; name: string }) {
  const [active, setActive] = useState(0);
  const [loaded, setLoaded] = useState<Record<number, boolean>>({});
  return (
    <div className="flex flex-col gap-4">
      {/* Main image - larger and with a thicker border */}
      <div className="relative aspect-[16/10] overflow-hidden border-[6px] border-foreground bg-muted shadow-[8px_8px_0_0_#c6633f]">
        ...
        />
        <div className="absolute bottom-4 right-4 bg-primary text-primary-foreground font-mono text-[10px] font-bold px-3 py-1 uppercase tracking-[0.2em]">
        ...
        {images.map((img, i) => (
          <button
            ...
            className={`relative aspect-[4/3] overflow-hidden border-2 cursor-pointer transition-all ${
              i === active ? "border-primary scale-[0.95] shadow-sm" : "border-foreground/20 hover:border-foreground/50"
            }`}
          >
...
export default function FeaturedListingPage() {
  ...
  return (
    <div className="pt-20">
      <SEO
        title={`Featured: ${listing.name}`}
        description={`Exclusive access to ${listing.name} in ${listing.neighborhood}.`}
      ...
      <main className="bg-background min-h-screen page-enter">
        ...
        <div className="border-b-[3px] border-foreground px-5 md:px-10 py-3 flex items-center justify-between">
          ...
          <span className="section-label text-primary font-bold uppercase tracking-widest">Featured Selection</span>
        </div>

        {/* Title block - Enhanced for Featured */}
        <section className="border-b-[3px] border-foreground px-5 md:px-10 py-8 md:py-12 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-end">
          <Reveal direction="up">
            <div>
              <p className="section-label text-primary mb-3 inline-flex items-center gap-1.5 font-bold uppercase tracking-widest">
                <MapPin className="w-3 h-3" /> {listing.neighborhood}, Mumbai
              </p>
              <h1 className="font-sans font-extrabold leading-[0.92] tracking-[-0.04em] mb-3" style={{ fontSize: "clamp(42px, 7vw, 84px)" }}>
                {listing.name}
              </h1>
              <p className="font-sans text-muted-foreground text-base md:text-lg max-w-xl italic leading-relaxed">
                "{listing.tagline}"
              </p>
            </div>
          </Reveal>
          <Reveal direction="left">
            <div className="border-[3px] border-foreground bg-primary text-primary-foreground p-6 bs min-w-[260px] shadow-[6px_6px_0_0_#2D2318]">
              <p className="section-label text-primary-foreground/70 mb-1">Monthly Rent</p>
              <p className="font-mono font-extrabold text-4xl tracking-tight">{listing.price}</p>
              <p className="flex items-center gap-1.5 font-mono text-xs text-primary-foreground/60 mt-2">
        ...
        {/* Stats strip */}
        <section className="border-b-[3px] border-foreground grid grid-cols-3 divide-x-[3px] divide-foreground bg-card">
          {[
            ...
            <div key={label} className="px-5 md:px-10 py-6 flex items-center gap-3">
              <Icon className="w-6 h-6 text-primary flex-shrink-0" />
              <div>
                <p className="section-label text-muted-foreground">{label}</p>
                <p className="font-sans font-bold text-xl">{value}</p>
              </div>
            </div>
          ))}
        </section>
        ...
        <section className="border-b-[3px] border-foreground px-5 md:px-10 py-10 md:py-14 grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-10">
          <Reveal direction="up">
            <FeaturedGallery images={listing.gallery} name={listing.name} />
          </Reveal>

          <RevealGroup stagger={0.1}>
            <div className="flex flex-col gap-6">
              <div>
                <h2 className="font-sans font-extrabold text-2xl tracking-tight mb-4 uppercase tracking-tight">The Residence</h2>
                <div className="flex flex-col gap-3">
                  {listing.description.map((p, i) => (
                    <p key={i} className="font-sans text-muted-foreground leading-relaxed text-lg">{p}</p>
                  ))}
                </div>
              </div>

              <div className="border-t-[2px] border-foreground/15 pt-5">
                <h3 className="section-label text-muted-foreground mb-3 font-bold">Exclusive Features</h3>
        ...
        <section className="border-b-[3px] border-foreground px-5 md:px-10 py-10 md:py-14 bg-card">
          <h2 className="font-sans font-extrabold text-2xl tracking-tight mb-6 uppercase">Building Amenities</h2>
          <RevealGroup stagger={0.1}>
        ...
        <section className="border-b-[3px] border-foreground px-5 md:px-10 py-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-foreground text-card">
          <div>
            <h2 className="font-sans font-extrabold text-2xl md:text-3xl tracking-tight mb-2">Secure this residence?</h2>
            <p className="font-sans text-card/60 max-w-md">Our Worli desk lead is available for a private viewing of this penthouse.</p>
          </div>
        ...
        <section className="px-5 md:px-10 py-12">
          <h2 className="font-sans font-extrabold text-2xl tracking-tight mb-6">Similar Residences</h2>
      ...
      <BookingModal
        ...
        propertyContext={`Featured Access: ${listing.name} (${listing.neighborhood})`}
      />
    </div>
  );
}
